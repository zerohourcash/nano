...
| Bit и бухгалтерия | кошелёк с балансом, переводом, продажей по ТМЦ, owner-emission и личной/аудиторской историей; scoped recipient directory без телефонов; двойная запись, deterministic conflict reconciliation, pre-trust binding проводки к device-signed Ledger | Playwright выполняет UI mint→transfer→sale, проверяет баланс, Ed25519 device-proof и accounting audit; Rust проверяет personal-history ACL/минимизацию каталога; `npm run adversarial:test` проверяет recipient/amount tamper | Проверено |
