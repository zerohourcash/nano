...
| Фото и документы | карточка имеет рабочую загрузку до 32 МБ с выбором ACL; scoped CAS ingest требует `itemId`/организацию/`manageDocuments`, затем маленький signed `photo_add`/`document_add` V3 intent связывает GUID карточки и вложения, CAS-хэш, MIME, автора, ACL и семантические поля; chunks, resume, metadata/smart/full | Playwright из вкладки карточки загружает accounting PDF, проверяет его отображение, CAS URL, ACL и Ledger V3; `npm run sync:test` физически останавливает upstream, создаёт PDF локально, восстанавливает full-node, проверяет bytes/ACL/rollback; Rust проверяет manager-without-create ACL, atomic rollback и trusted-node rewrite | Проверено для новых вложений; старые записи явно legacy |
