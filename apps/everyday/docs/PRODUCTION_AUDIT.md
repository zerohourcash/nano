...
| --- | --- | --- | --- |
| Подписанная выдача по QR | tenant-bound QR V2 с Ed25519 node-proof, проверкой доверия и явным legacy-режимом; Ed25519 device-proof операции, Ledger V2, append-only custody commitment и восстановление holdings. Физический клон подлинной бирки не считается криптографически обнаружимым | `npm run test:e2e`, `npm run sync:test`, Rust QR foreign-tenant/rebind/tamper/custody tests | Проверено |
...
| Антиспам внутреннего чата | device-proof, 4000 символов, 10 вложений, быстрый duplicate suppression и persisted 20 сообщений/мин на workspace+member; отказ атомарен и не создаёт Ledger-полусобытие | `npm run spam:test` атакует release-узел через реальный HTTP, проверяет дубль/21-е сообщение/oversize/SQLite counts/restart; Rust проверяет изоляцию лимита другого участника | Проверено |
...
| 100 узлов | разреженная топология, 100 процессов/БД, отказ и cold restart 10 процессов; журнал и CAS догоняются после восстановления | `npm run scale:test`: 100/100 за 20,8 с, leaf→root за 6,5 с, peer limit 4, RSS p95 12,3 MiB | Проверено 2026-09-01 |
