...
| Подписанный акт инвентаризации | завершённая сессия выгружается как portable JSON с каноническими байтами, SHA-256, Ed25519 node-proof и ссылками на device-signed records | Rust проверяет байты/hash/signature; Playwright скачивает акт через UI и независимо проверяет SHA-256 + Ed25519 в Chromium | Проверено |
