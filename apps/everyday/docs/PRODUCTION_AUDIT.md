...
| Desktop Linux/Windows/macOS | один Rust/SQLite backend и offline PWA упаковываются с платформенным launcher и SHA-256 manifest v2; manifest связывает пакет с точным Git revision, dirty tracked tree запрещён. Пакет запрещает runtime DB/keys/APK и проходит реальный startup/health/UI smoke | `npm run desktop:package` сверяет revision с checkout и отдельным tamper-тестом доказывает отказ после его замены; GitHub `desktop-release` matrix выполняет native Rust tests/build/manifest/smoke на `ubuntu-latest`, `windows-latest`, `macos-latest` и сохраняет отдельные artifacts | Linux x86_64 проверен локально; Windows/macOS требуют зелёного CI run |
