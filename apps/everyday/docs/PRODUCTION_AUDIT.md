...
| Инвентаризация offline → owner | создание, каждая QR-сверка и завершение — device-signed portable records; исходные/фактические количества и участник восстанавливаются на полной ноде, node-signed подмена отклоняется | `npm run smoke`, `npm run sync:test`, Rust `signed_inventory_round_trip_and_tamper_rejection` | Проверено |
