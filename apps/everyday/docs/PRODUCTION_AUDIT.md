...
| Межорганизационный mesh-конверт | scoped-журналы не смешиваются; gateway identity, QR-визитка, доверенный каталог, inbox, opaque X25519 + HKDF-SHA256 + XChaCha20-Poly1305, Ed25519 sender proof, TTL/replay/PoW/quota, signed send/accept | `npm run interorg:test`: 2 процесса/БД без общего token, физический partition/restart, ciphertext inspection и delivery; Rust adversarial tests; Playwright V3 address; QR/MTU tests | Проверено для текстовых транзакций |
