...
| Межорганизационный mesh-конверт | scoped-журналы не смешиваются; gateway identity, QR-визитка, каталог с signed rotate/revoke, inbox, opaque X25519 + HKDF-SHA256 + XChaCha20-Poly1305, Ed25519 sender proof, TTL/envelope replay/semantic transaction replay/PoW/quota, signed send/accept | `npm run interorg:test`: 2 процесса/БД без общего token, physical partition/restart, ciphertext inspection, новый envelope с тем же transaction ID, delivery и post-revoke rejection; Rust adversarial/migration tests; Playwright V3 address/revoke; QR/MTU tests | Проверено для текстовых транзакций |
...
| Bit и бухгалтерия | кошелёк с балансом, переводом, продажей только ответственному за ТМЦ, owner-emission и личной/аудиторской историей; scoped recipient directory без телефонов; двойная запись, deterministic conflict reconciliation, pre-trust binding проводки к device-signed Ledger | Playwright выполняет UI mint→transfer→sale, проверяет seller spoof без полупроводки, баланс, Ed25519 device-proof и accounting audit; Rust проверяет seller binding, atomic rollback, personal-history ACL/минимизацию каталога; `npm run adversarial:test` проверяет recipient/amount tamper | Проверено |
