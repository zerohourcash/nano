...
| Подписанный акт инвентаризации | завершённая сессия выгружается как portable JSON с каноническими байтами, SHA-256, Ed25519 node-proof и ссылками на device-signed records; принимающая нода различает trusted/untrusted key, чужую организацию, missing/mismatched history | Rust проверяет байты/hash/signature, подмену, неизвестный ключ, отсутствующую историю и tenant isolation; Playwright скачивает, независимо проверяет SHA-256 + Ed25519, загружает оригинал и подделку через UI | Проверено |
