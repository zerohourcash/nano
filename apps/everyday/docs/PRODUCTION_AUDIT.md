...
75 725 388 байт, SHA-256
`7755df1830952ef09d25446ca2bbf540dec7e497a783b525f9b460342a84ba42`.
...
fail-closed запуск до создания базы при подменённом пакете командой
...
`sourceRevision=7ceb12fbee30cc423c7c80396563e552f1755810` и
`sourceDirty=false`; runtime `/health.buildRevision` совпадает с manifest.
Архив имеет размер 28 070 785 байт и SHA-256
`2ac43691e3f4433f0a554d5ab02bedf51386d766e89148ab7d60ef8ea005e55c`.
...
остаётся парольным и invite-only.
Публичный `:8443` reverse proxy дополнительно проходит browser login regression
с явным `MESHKEEPER_PUBLIC_ORIGIN`; запрос с чужим Origin по-прежнему получает
403, а forwarded-заголовки не участвуют в решении.
Панель владельца «Версия и контрольные суммы» показывает embedded
`/health.buildRevision` и SHA-256 опубликованных APK/desktop. Production
Playwright проверяет реальную revision Rust-ноды и явное состояние, когда
артефакт на изолированной ноде не опубликован.

| Требование | Реализация | Исполняемое доказательство | Статус |
| --- | --- | --- | --- |
| Обязательная подтверждённая выдача по QR | API принимает tenant-bound QR V2 либо готовую внешнюю метку, чья точная уникальная привязка находится в текущей device-signed `item-state/v1` версии карточки. Для V2 сервер проверяет Ed25519 node-proof, организацию, item binding и tenant-scoped доверие signer key. Непривязанный legacy, зарезервированный или неоднозначный после offline-слияния код запрещён; каждая штучная единица сканируется отдельно. Точное тело коммитится device-proof в Ledger V3, публичная история раскрывает только SHA-256 QR proof. Append-only custody детерминированно восстанавливает holdings, ответственного, статус и срок; две положительные offline-ветви дают `needs-check`. Внешняя привязка подтверждает решение организации, но физический клон или подлинность производителя криптографически не доказывает | Rust проверяет создание, versioned lookup, case-insensitive duplicate, reserved prefix, wrong-code и успешную выдачу; Playwright привязывает готовый QR через UI, отклоняет непроверенный legacy и выдаёт ТМЦ по внешней метке; `npm run smoke` проверяет no-QR/wrong/tamper/ACL/digest; `npm run sync:test` выполняет offline join и обратную сходимость | Проверено |
...
| Desktop Linux/Windows/macOS | один Rust/SQLite backend и offline PWA упаковываются с платформенным launcher и SHA-256 manifest v2; manifest связывает пакет с точным Git revision, dirty tracked tree запрещён, revision встроена в Rust binary. Пакет запрещает runtime DB/keys/APK и проходит реальный startup/health/UI smoke | `npm run desktop:package` сверяет revision с checkout, tamper-тестом доказывает отказ после её замены и требует равенства runtime `/health.buildRevision`; GitHub `desktop-release` matrix выполняет native Rust tests/build/manifest/smoke на `ubuntu-latest`, `windows-latest`, `macos-latest` | Linux x86_64 проверен локально; Windows/macOS требуют зелёного CI run |
...
| Конфликты/двойная выдача | обе custody-ветви сохраняются, предмет → `needs-check`; API списка и решения ограничен активной организацией, ID чужого конфликта не обходит ACL. Решение аудитора — отдельный идемпотентный Ledger V3 intent с UUID, workspace/item/responsible GUID и commitment; оно сбрасывает конфликтный custody baseline и переносимо закрывает конфликт даже на ноде без локальной строки detection | `npm run adversarial:test` выполняет partition → двойную QR-выдачу → quarantine → signed resolve → reconnect и получает одинаковый `in-stock`/resolved на двух БД; Rust проверяет ACL, bounded intent и отклоняет подмену resolution commitment | Проверено |
| Подделка/replay | snapshot hash, Ed25519, nonce, portable device registry/revoke tombstone, node trust, signed monotonic sequence/scope, atomic receipt, rollback savepoint. Fail-closed contract не позволяет browser/backend/HTTP-test signer разойтись или добавить mutation без явного решения о подписи | `npm run security:contract`, `npm run adversarial:test`, Rust device-binding/rollback/equivocation tests, `npm run verify` | Проверено |
| Несколько организаций | scoped capability journal/CAS и отдельные `(node key, workspace GUID)` области доверия QR; chat/wiki/item/custody/fault/writeoff manifests и blob ACL вычисляются независимо, знание чужого SHA-256 не даёт файл; UI списка участников привязан к выбранной организации; должность, роль, табельный номер, права и политика выдачи входят в versioned membership, тогда как tenant-admin не может менять глобальные PII/статус | `npm run capability:test`: два bearer и два full-peer получают только свои workspace, QR key scope, chat CAS и фото списания, чужой blob возвращает 403; concurrent role/revoke в `npm run adversarial:test`; Rust multi-organization policy/API, valid offline policy replication, forged policy hash, scope/import/CAS/QR tests; Playwright сохраняет и перечитывает организационную роль и лимит выдачи | Проверено |
| Межорганизационный mesh-конверт | scoped-журналы не смешиваются; gateway identity, QR-визитка, каталог с signed rotate/revoke, inbox/outbox, opaque X25519 + HKDF-SHA256 + XChaCha20-Poly1305, Ed25519 sender proof, TTL/envelope replay/semantic transaction replay/PoW/quota, signed send/accept и обратная encrypted-квитанция с independently verified V3 device+node proof. `message.file` переносит внутри AEAD файл до 24 КиБ со строгими name/MIME/size/SHA-256/base64; relay не видит метаданные или байты. Android BLE использует отдельный transport kind `3`; desktop/PWA передаёт тот же batch системным Share или файлом через USB/SD/Wi-Fi Direct. Пользовательский batch ≤32 tenant-scoped envelope и ≤3 МБ, exact API route отклоняет oversized body HTTP 413 до JSON/signature/SQLite, signed import атомарен и durable spool подтверждается только после проверки. Новый inbox хранит source envelope; полный аудит повторно открывает AEAD и сверяет точный payload и V3 accept | `npm run interorg:test`: 2 процесса/БД без общего token и relay URL, ранний HTTP 413, physical partition/restart, явный BLE/file-совместимый gossip carry в обе стороны, ciphertext inspection, текст и реальный бинарный файл, проверка точных байтов/SHA-256, локальная SQL-подмена, delivery/accept/receipt/replay/post-revoke; Rust tenant-scope/atomic rollback/bounds/hash/confidentiality tests; Android contract+instrumentation payload-kind test; Playwright выполняет signed empty-file import, отвергает 33 envelope и показывает BLE/file actions | Проверено для новых текстовых и малых файловых транзакций; старый inbox explicit legacy |
| Административная летопись | участники, роли, приглашения и дерево требуют device-proof и пишутся атомарно в Ledger V2; capability-вступление атомарно объединяет создание аккаунта, membership, расход invite и Ledger, а повтор уже состоящего участника не расходует invite; importer проверяет тип события, actor, target GUID и полноту proof | `npm run smoke`, `npm run sync:test` с offline join → QR → owner, Rust forced-Ledger-failure rollback, membership/device/wrong-target tests | Проверено |
...
| Структура организации после offline-разрыва | разделы любой глубины — device-signed portable branches; parent/responsible передаются по GUID, произвольный type сохраняется без подмены, конфликт сходится по `(depth, versionHash)`, циклы и node-signed подмена снимка запрещены. UI создаёт, переименовывает и переносит разделы, назначает ответственного по ФИО и исключает собственное поддерево из родителей. Непустой `tabLabel` создаёт реальную вкладку каталога; backend tenant-check + recursive CTE показывает ТМЦ всего активного поддерева и отклоняет чужой/архивный root | Rust `signed_organization_tree_replication_rejects_snapshot_rewrite`; API custom kind/responsible/descendant/foreign filter и cycle tests; Playwright создаёт подразделение→кабинет, помещает ТМЦ в leaf, проверяет вкладку, подписанно меняет её type/имя/ответственного и видит новое имя в каталоге; integrity audit | Проверено |
...
| 100 узлов | разреженная топология, 100 процессов/БД, обязательная QR V2 выдача на leaf, отказ и cold restart 10 процессов; журнал и CAS догоняются после восстановления. После повторных push/pull тест сравнивает полный набор `(opId, hash)`, текущее состояние и `messageGuid` всех БД, запрещает дубли и запускает криптографический audit на каждом узле | `npm run scale:test`: свежий прогон 2026-09-01 — 100/100, первичная сходимость 23,2 с, leaf→root 6,2 с, cold restart 10/10, полный audit 100/100, peer limit 4, RSS p95 12,6 MiB | Проверено 2026-09-01 |
...
| Пополнение, списание, остаток и перемещение | точный device-signed Ledger V3 intent связывает UUID операции, организацию, ТМЦ, количество, комментарий и GUID назначения; повтор UUID и ссылка на склад/объект чужой организации отклоняются. Импорт связывает последнее перемещение с текущим местом, а количественный остаток детерминированно восстанавливает из последней signed master-величины, складских delta и custody-выдач/возвратов; входной snapshot не является источником истины | Rust отклоняет замену количества, назначения, итогового местоположения или остатка при неизменной летописи и отдельно восстанавливает `10 − 3 = 7` из master+custody вместо подложенного `99`; `npm run sync:test` проверяет реальную offline-выдачу 4 из 10 и восстановление остатка 6 | Проверено для новых операций; старые записи явно legacy |
...
| Bit и бухгалтерия | кошелёк, direct mint/transfer/sale с compact Ledger V3 user intent и portable двухфазная продажа штучных ТМЦ/части партии: seller offer → partition sync → buyer accept/reject → обратная сходимость Bit + custody. Audit считает V3 и legacy отдельно | Playwright проверяет V3 mint/transfer/sale и предложение 4 из 6; Rust отклоняет trusted-node recipient substitution при валидных device/node proofs, проверяет partial holdings 6→2+4, round-trip, цену/количество, replay, insufficient funds, stale seller/seller spoof; `npm run sync:test` подтверждает V3 intent на owner после разрыва | Проверено для новых операций; V2 явно legacy |
| Android | тот же Rust backend, Keystore, private UI/sync-only LAN; build revision встраивается в обе JNI ABI и сверяется с checkout | APK verifier, `npm run mobile:node:test` | Проверено ARM64/x86_64 |
...
| Android BLE GATT | foreground advertiser/server + scanner/client, MTU/retry, bounded crash-safe ciphertext spool, authorized import handoff, persistent diagnostics. Различает encrypted org sync kind `1` и opaque interorg gossip kind `3`; повреждённый claimed payload атомарно уходит в quarantine/retry и не подтверждается до Rust API import | Rust API regression, Android app+instrumentation APK compile, тест claim/retry/accept/bounds/quarantine и двух payload kinds, API 34 KVM CI gate, APK | Реализовано; локальная software-emulation без KVM не загрузилась, внешний успешный instrumentation run ещё не зафиксирован; RF-тест на двух телефонах не выполнен |
...
Android-сборка использует сохранённый в репозитории Gradle Wrapper 8.7 через
кроссплатформенный `scripts/run-gradle.mjs`; глобально установленный `gradle`
не требуется. SDK задаётся стандартными `ANDROID_HOME`/`ANDROID_SDK_ROOT`.
