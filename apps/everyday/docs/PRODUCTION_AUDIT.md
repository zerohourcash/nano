...
| Несколько организаций | scoped capability journal/CAS; chat/wiki/item/custody manifests и blob ACL вычисляются независимо, знание чужого SHA-256 не даёт файл; versioned membership ACL и revoke tombstones | `npm run capability:test`: два bearer и два full-peer получают только свои workspace и chat CAS, чужой blob возвращает 403; concurrent role/revoke в `npm run adversarial:test`; Rust scope/import/CAS tests | Проверено |
