...
| Межорганизационный mesh-конверт | scoped-журналы не смешиваются; gateway identity, QR-визитка, каталог с signed rotate/revoke, inbox/outbox, opaque X25519 + HKDF-SHA256 + XChaCha20-Poly1305, Ed25519 sender proof, TTL/envelope replay/semantic transaction replay/PoW/quota, signed send/accept и обратная encrypted-квитанция с correlation + Ledger hash | `npm run interorg:test`: 2 процесса/БД без общего token, physical partition/restart, ciphertext inspection, delivery, accept, обратный gossip receipt, новый envelope с тем же transaction ID и post-revoke rejection; Rust отвергает receipt для неизвестной операции даже от trusted peer; Playwright V3 address/revoke; QR/MTU tests | Проверено для текстовых транзакций |
