...
| Инвентаризация offline → owner | камера/Android bridge/фото декодируют реальный QR и принимают только GUID/номер позиции текущей сессии; создание, каждая QR-сверка и завершение — device-signed portable records; Ledger V3 переносит подписанное request body, поэтому проверяются не только hashes, но и соответствие `actualQty/checked` пользовательскому intent | Playwright экспортирует PNG-бирку, декодирует её через `jsQR`, отклоняет чужой GUID и проверяет device-proof; `npm run smoke`, `npm run sync:test`, Rust snapshot-tamper и semantic-substitution в `signed_inventory_round_trip_and_tamper_rejection` | Проверено |
