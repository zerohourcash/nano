...
| Фото и документы | двухфазный CAS ingest → маленький signed `photo_add`; V3 intent связывает item/photo GUID, original/thumbnail hash и title-флаг; chunks, resume, metadata/smart/full | Playwright реальная загрузка из браузера, `npm run smoke`, `npm run sync:test`, Rust atomic rollback и `photo_intent_rejects_trusted_node_snapshot_rewrite` | Проверено для новых фото; старые снимки явно legacy |
