...
| Межорганизационный mesh-конверт | scoped-журналы не смешиваются; gateway identity, QR-визитка, доверенный каталог, inbox, opaque X25519 + HKDF-SHA256 + XChaCha20-Poly1305, Ed25519 sender proof, TTL/replay/PoW/quota, signed send/accept | Rust partitioned three-hop и trusted-organizations tests, API ACL/Ledger test, browser creates signed V3 address, QR unit tests, transport MTU round-trip | Проверено для текстовых транзакций |
