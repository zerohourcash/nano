...
| Bit и бухгалтерия | кошелёк с балансом, прямым платежом и portable двухфазной продажей: seller offer → partition sync → buyer accept/reject → обратная сходимость Bit + custody; owner-emission, scoped directory и deterministic conflict reconciliation | Playwright выполняет UI mint→transfer→seller offer→sale; Rust поднимает две независимые БД, переносит pending, принимает на buyer-node, возвращает accepted продавцу и проверяет балансы/ответственного; отклоняет подмену цены с пересчитанным hash и новой node-signature, replay, insufficient funds, stale seller и seller spoof | Проверено |
