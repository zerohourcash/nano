...
| Пополнение и перемещение | точный device-signed Ledger V3 intent связывает UUID операции, организацию, ТМЦ, количество, комментарий и GUID назначения; повтор UUID и ссылка на склад/объект чужой организации отклоняются. Импорт повторно вычисляет commitment и связывает последнее перемещение с `storageGuid/buildingSiteGuid` текущего снимка, пока его не заменит более новая signed master-версия | Rust проверяет bounded request retention и отклоняет замену количества, назначения или только итогового местоположения при неизменной летописи; полный backend suite и Playwright production build | Проверено для новых операций; старые записи явно legacy |
