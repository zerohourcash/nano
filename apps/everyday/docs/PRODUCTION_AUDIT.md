...
| 100 узлов | разреженная топология, 100 процессов/БД, обязательная QR V2 выдача на leaf, отказ и cold restart 10 процессов; журнал и CAS догоняются после восстановления. После повторных push/pull тест сравнивает полный набор `(opId, hash)`, текущее состояние и `messageGuid` всех БД, запрещает дубли и запускает криптографический audit на каждом узле | `npm run scale:test`: усиленный прогон 2026-09-01 — 100/100, первичная сходимость 22,7 с, leaf→root 10,1 с, полный audit 100/100, peer limit 4, RSS p95 12,6 MiB | Проверено 2026-09-01 |
