...
| Пополнение и перемещение | точный device-signed Ledger V3 intent связывает UUID операции, организацию, ТМЦ, количество, комментарий и GUID назначения; повтор UUID и ссылка на склад/объект чужой организации отклоняются, импорт повторно вычисляет commitment | Rust проверяет bounded request retention и отклоняет замену количества/назначения; полный backend suite и Playwright production build | Проверено для новых операций; старые записи явно legacy |
