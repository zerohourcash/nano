...
| Чат и wiki | ledger-bound chat; revision DAG, двухфазный CAS и V3 user intent для slug/title/text/ACL/parent/attachments с explicit legacy | Playwright browser upload, `npm run mesh:test`, `npm run sync:test`, Rust trusted-node rewrite test | Проверено для новых ревизий |
