...
| Bit и бухгалтерия | кошелёк с балансом, прямым платежом и двухфазной продажей: seller offer → buyer accept → атомарные Bit + custody; owner-emission и личная/аудиторская история; scoped directory без телефонов; deterministic conflict reconciliation | Playwright выполняет UI mint→transfer→seller offer→sale и проверяет отсутствие раннего списания; Rust проверяет две стороны, replay, insufficient funds, stale seller, seller spoof и atomic rollback; adversarial test проверяет recipient/amount tamper | Онлайн и на общем локальном узле проверено; portable pending-offer между разделёнными узлами — открытый gate |
