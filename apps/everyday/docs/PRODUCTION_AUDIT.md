...
| Межорганизационный mesh-конверт | scoped-журналы не смешиваются; gateway identity, доверенный каталог контрагентов, opaque X25519 + HKDF-SHA256 + XChaCha20-Poly1305, Ed25519 sender proof, TTL/replay/PoW/quota, signed send/accept | Rust partitioned three-hop и trusted-organizations tests, API ACL/Ledger test, transport MTU round-trip | Backend-поток проверен; UI каталога ещё не подключён |
