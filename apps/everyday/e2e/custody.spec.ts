    ...
    async ({ procedure, input, mutation }) => {
      ...
      if (mutation) {
        ...
        const state = window as typeof window & {
          __everydayE2EIdentity?: Identity;
        };
        ...
        if (!registration.ok)
          throw new Error(`device registration: HTTP ${registration.status}`);
      ...
      const response = await fetch(`/api/trpc/${procedure}${suffix}`, {
        ...
        headers: mutation
          ? { "content-type": "application/json", ...signedHeaders }
          : undefined,
...
test("browser signs a real custody transaction and ledger retains its proof", async ({
  ...
  await expect(
    page.getByText(/Не добавлены: empty\.pdf.*1 байт–20 МБ/)
  ).toBeVisible();
  await createDocumentInput.setInputFiles({
    name: "manual.pdf",
    mimeType: "application/pdf",
  ...
  await expect(
    page.getByRole("heading", { name: /Сервис E2E \(1 ед\.\)/ })
  ).toBeVisible();
  await expect(page.getByText("Перфоратор E2E", { exact: true })).toBeVisible();
  await page.goto("/admin");
  await page
    .getByRole("button", { name: "Структура", exact: true })
    .first()
    .click();
  await page
    .getByRole("button", { name: "Изменить Сервисное подразделение" })
    .click();
  ...
  await page
    .getByLabel("Ответственный за раздел")
    .selectOption({ label: "Елена Владелец" });
  await page.getByRole("button", { name: "Сохранить", exact: true }).click();
  await expect(
    page.getByText("Изменения раздела подписаны и сохранены")
  ).toBeVisible();
  await expect(
    page.getByText(/мастерская.*ответственный: Елена Владелец/)
  ).toBeVisible();
  await page
  ...
  await expect(
    page.getByText("Главный инженер E2E", { exact: true })
  ).toBeVisible();
  await page.getByRole("button", { name: "Действия" }).first().click();
  await page.getByRole("menuitem", { name: "Права доступа" }).click();
  ...
  await page.goto("/");
  await expect(
    page.getByRole("button", { name: "Ремонт E2E", exact: true })
  ).toBeVisible();
  ...
  const itemQr = await trpc<{ label: string }>(
    page,
    "items.qrLabel",
    { itemId },
    false
  ...
  await expect(
    page.getByText("QR получен и будет проверен сервером")
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Документ сохранён в CAS и подписан")
  ).toBeVisible({
    timeout: 10_000,
  });
  await expect(
    page.getByText("invoice-e2e.pdf", { exact: true })
  ).toBeVisible();
  ...
  const photoEvent = itemAfterTake.history.find(
    entry => entry.type === "photo_add"
  );
  ...
  const documentEvent = itemAfterTake.history.find(
    entry => entry.type === "document_add"
  );
  ...
  await expect(page.locator("#item-qr-canvas")).toBeAttached({
    timeout: 10_000,
  });
  ...
  const inventorySite = await trpc<{ id: number }>(
    page,
  ...
  await trpc(page, "items.update", {
    id: qrItem.id,
    buildingSiteId: inventorySite.id,
  });
  await page.goto("/inventory");
  await page
    .getByRole("button", { name: "Новая инвентаризация" })
    .first()
    .click();
  await page.getByRole("button", { name: "Объект", exact: true }).click();
  await page
    .locator("select")
    .filter({ has: page.locator(`option[value="${inventorySite.id}"]`) })
    .selectOption(String(inventorySite.id));
  await page
    .getByText("Блокировать передачи в области сверки до завершения")
    .click();
  ...
  const inventoryCode = inventoryScanner.getByPlaceholder(
    "Или вставьте ссылку / токен"
  );
  await inventoryCode.fill(
    "everyday:item:00000000-0000-4000-8000-000000000000"
  );
  await inventoryScanner.getByRole("button", { name: "Далее" }).click();
  await expect(
    page.getByText("QR не прошёл проверку или относится к другой организации")
  ).toBeVisible();
  ...
  await expect(page.getByText("Отмечено: Шуруповёрт QR E2E")).toBeVisible({
    timeout: 10_000,
  });
  await inventoryScanner
    .getByRole("button", { name: "Закрыть сканер" })
    .click();
  ...
  expect(
    inventoryAfterScan.results.find(result => result.itemId === qrItem.id)
  ).toMatchObject({
    checked: true,
    actualQty: 1,
  });
  const inventoryHistory = await trpc<
    Array<{
      type: string;
      eventVersion: number;
      requestDeviceId?: string;
  ...
  expect(
    inventoryHistory.find(entry => entry.type === "inventory_check")
  ).toMatchObject({
    eventVersion: 3,
  ...
  await page
    .getByRole("button", { name: "Скачать подписанный акт" })
    .last()
    .click();
    ...
    await page.evaluate(async document => {
      ...
      const valid = await crypto.subtle.verify(
        ...
        new TextEncoder().encode(
          `${document.signatureDomain}\n${document.hash}`
        )
      );
      return (
        hash === document.hash &&
        valid &&
        JSON.stringify(canonicalAct) === JSON.stringify(document.act)
      );
    }, signedAct)
  ).toBe(true);
  ...
  await expect(page.getByTestId("inventory-act-verification")).toContainText(
    "Акт полностью подтверждён"
  );
  const tamperedAct = structuredClone(signedAct) as typeof signedAct & {
  ...
  await expect(page.getByTestId("inventory-act-verification")).toContainText(
    "Подпись или содержимое акта повреждены"
  );
  ...
  const offeredQr = (await trpc(
    page,
    "items.qrLabel",
    { itemId: offeredItem.id },
    false
  )) as { label: string };
  await trpc(page, "transfers.take", {
    itemId: offeredItem.id,
    qrLabel: offeredQr.label,
    quantity: 6,
  });
  await page.goto("/bit");
  await expect(
    page.getByRole("heading", { name: "Кошелёк Bit" })
  ).toBeVisible();
  ...
  await page
    .getByLabel("Назначение платежа")
    .fill("Подписанное предложение E2E");
  await page.getByRole("button", { name: "Подписать транзакцию" }).click();
  ...
  const pendingOffers = await trpc<
    Array<{
      id: number;
      status: string;
      bitAmount: number;
  ...
  expect(
    materialBeforeAcceptance.holders.find(
      holder => holder.userId === currentUser.id
    )?.quantity
  ).toBe(6);
  ...
  const bitTransactions = await trpc<
    Array<{
      kind: string;
      status: string;
  ...
  expect(
    bitTransactions.every(transaction => transaction.status === "posted")
  ).toBe(true);
  expect(
    bitTransactions.find(transaction => transaction.kind === "sale")
  ).toMatchObject({
    senderUserId: currentUser.id,
    recipientUserId: bitRecipient.id,
  });
  const bitHistory = await trpc<
    Array<{
      type: string;
      eventVersion: number;
      requestDeviceId?: string;
  ...
  await page
    .getByPlaceholder("Сообщение группе…")
    .fill("Файл из локального чата");
  ...
  await expect(
    page.getByText(/Пакет проверен: новых 0, доставлено 0, повторов 0/)
  ).toBeVisible();
  ...
  const interorgHistory = await trpc<
    Array<{
      type: string;
      eventVersion: number;
      requestDeviceId?: string;
  ...
  >(page, "history.all", { workspaceId: workspaces[0].id, limit: 500 }, false);
  expect(
    interorgHistory.find(entry => entry.type === "interorg_identity_create")
  ).toMatchObject({
    eventVersion: 3,
    requestDeviceId: expect.any(String),
    requestHash: expect.stringMatching(/^[a-f0-9]{64}$/),
  ...
  const e2eContact = await trpc<{ guid: string }>(
    page,
  ...
  await page.reload();
  await page.getByRole("button", { name: "Сеть организаций" }).first().click();
  await expect(page.getByTestId("interorg-contact")).toContainText(
    "E2E Контрагент"
  );
  ...
  await page.reload();
  await page.getByRole("button", { name: "Сеть организаций" }).first().click();
  const outgoingInterorg = page
    .getByTestId("interorg-outbox-item")
    .filter({ hasText: interorgTransactionId });
  ...
  await page
    .getByPlaceholder("Текст сообщения или условия сделки")
    .fill("Закрытый файл E2E");
  ...
  await expect(
    page.getByTestId("interorg-outbox-item").filter({ hasText: "message.file" })
  ).toContainText("Ожидает квитанцию");
  ...
  const provenance = page.getByTestId("release-provenance");
  await expect(provenance).toContainText("Версия и контрольные суммы");
  await expect(provenance).toContainText(/Rust-нода: [0-9a-f]{40}(?:-dirty)?/);
  await expect(provenance).toContainText(
    "Android APK: не опубликован на этой ноде"
  );
});
