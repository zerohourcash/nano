...
const manifest = JSON.parse(
  readFileSync(path.join(release, "release-manifest.json"), "utf8")
);
const expectedBuildRevision = `${manifest.sourceRevision}${manifest.sourceDirty ? "-dirty" : ""}`;
...
const temporary = mkdtempSync(
  path.join(os.tmpdir(), "everyday-desktop-smoke-")
);
...
try {
  ...
  for (let attempt = 0; attempt < 300; attempt += 1) {
    if (child.exitCode !== null)
      throw new Error(`Packaged node exited early: ${stderr}`);
    try {
      const response = await fetch(`http://127.0.0.1:${port}/health`);
      const body = response.ok ? await response.json() : null;
      healthy = response.ok && body?.buildRevision === expectedBuildRevision;
      if (healthy) break;
    } catch {
  ...
  if (!healthy)
    throw new Error(`Packaged node did not become healthy: ${stderr}`);
  const page = await fetch(`http://127.0.0.1:${port}/`);
  const html = await page.text();
  if (!page.ok || !html.includes('<div id="root">'))
    throw new Error("Packaged offline UI is unavailable");
