...
print("\n== 11. Неисправность ==")
unsigned_comment = owner.call("items.addComment", {"itemId": item_id, "text": "Офлайн-заметка"}, signed=False)
check("unsigned item comment rejected", unsigned_comment.get("__http") == 403 and "DEVICE_SIGNATURE_REQUIRED" in unsigned_comment.get("__body", ""), str(unsigned_comment)[:160])
comment = owner.call("items.addComment", {"itemId": item_id, "text": "Офлайн-заметка: проверить патрон"})
check("item comment is ledger-bound", isinstance(comment, dict) and bool(comment.get("recordHash")) and bool(comment.get("ledgerHash")), str(comment)[:180])
unsigned_fault = owner.call("items.reportFault", {"itemId": item_id, "severity": "high", "description": "Подмена"}, signed=False)
check("unsigned fault report rejected", unsigned_fault.get("__http") == 403 and "DEVICE_SIGNATURE_REQUIRED" in unsigned_fault.get("__body", ""), str(unsigned_fault)[:160])
...
unsigned_change = owner.call("items.requestChange", {"itemId": item_id, "payload": {"title": "Подмена"}}, signed=False)
check("unsigned change request rejected", unsigned_change.get("__http") == 403 and "DEVICE_SIGNATURE_REQUIRED" in unsigned_change.get("__body", ""), str(unsigned_change)[:160])
owner_change = owner.call("items.requestChange", {"itemId": item_id, "payload": {"title": "Перфоратор Bosch GBH 2-26"}, "comment": "Проверенная модель"})
check("change request is ledger-bound", bool(owner_change.get("recordHash")) and bool(owner_change.get("ledgerHash")), str(owner_change)[:180])
unsigned_decision = owner.call("items.decideChange", {"id": owner_change.get("id"), "accept": True}, signed=False)
check("unsigned change decision rejected", unsigned_decision.get("__http") == 403 and "DEVICE_SIGNATURE_REQUIRED" in unsigned_decision.get("__body", ""), str(unsigned_decision)[:160])
decision = owner.call("items.decideChange", {"id": owner_change.get("id"), "accept": True, "reason": "Модель подтверждена"})
