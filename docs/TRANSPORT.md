...
затем получает встречный delta. Journal v2 содержит подписанные
`journalSequence` и `journalScope`. Поэтому старый, но криптографически
корректный snapshot отклоняется до импорта; точный транспортный
дубль отвечает идемпотентным `duplicate=true`.

Права участников не выводятся из полноты snapshot: каждое членство
имеет версию и подписанную Ledger-ссылку. Отзыв передаётся как tombstone,
поэтому старая или изолированная нода не воскрешает удалённый ACL при
восстановлении связи. При одинаковой revision отзыв побеждает конкурирующее
изменение роли; порядок доставки пакетов не влияет на итог.
...
При заданном `MESHKEEPER_SYNC_WORKSPACES` зашифрованный пакет содержит только
разрешённые организации; пакет с дополнительной организацией отклоняется после
AEAD/Ed25519-проверки и до изменения SQLite.
