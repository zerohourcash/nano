...
const server=http.createServer(async(req,res)=>{try{
  ...
  if(req.method==='GET'&&u.pathname==='/api/diagnostics')return json(res,200,diagnostics.snapshot({ledger,peers,dataDir:DATA}));
  if(req.method==='DELETE'&&u.pathname==='/api/diagnostics/errors'){diagnostics.clear();return json(res,200,{ok:true})}
...
udp.on('message',(buf,r)=>{try{const m=JSON.parse(buf),{tag,...unsigned}=m;if(MESH_SECRET&&!safeEqual(tag,mac(unsigned,MESH_SECRET)))return;if(m.protocol!=='nano-inventory/1'||m.id===ledger.identity.actor)return;const p={id:m.id,name:m.name,host:r.address,port:m.port,lastSeen:new Date().toISOString()};peers.set(m.id,p);sync(p.host,p.port)}catch(e){diagnostics.record('warning','mesh',e.message,r.address)}});
...
process.on('unhandledRejection',e=>diagnostics.record('error','promise',e?.message||e,e?.stack));
